...
**For context, an earlier design.** Before the relay existed, the agent asked for email, phone, and (briefly) a password directly in the chat thread — functional, and exactly the trust problem described above. Shown here with a fabricated test identity, end to end through to a real LPA activation code, purely to illustrate what the current relay replaced:
