...
**LLM-as-judge conversation evaluation.** A scheduled job pulls unreviewed production conversations and scores each on intent clarity, response accuracy, tool selection, and grounding quality — storing structured feedback alongside concrete prompt and tool-docstring improvement suggestions for human review. The evaluator surfaces where behaviour degraded, a human decides what changes.
...
Coverage spans ten core user journeys across **49 LLM-judged scenarios** in three suites, plus edge cases, multi-tool chains, stress and loop-guard scenarios, the full authentication lifecycle, memory and context anchoring, and a dedicated long-horizon test validating the 90-day session lifecycle.
...
|---|---|---|
| **Unit and contract** | 159 tests — prompt vs toolset, training vs inference, client vs backend wire format, scrubber scores vs thresholds | Inside the container, no network |
| **Security regression** | Injection classifier, behavioural gate, honeypot, stream workers, voice pipeline | Real ONNX weights, live Redis and Bloom filters |
| **Journey integration** | 49 LLM-judged scenarios, identity merge, history anchoring, 90-day session lifecycle | Live stack, real LLM, real backend sandbox |
| **End-to-end driver** | Genuine platform webhooks through every stage of the pipeline | Running app plus an in-network platform API stub |
| **Load and adversarial** | Locust scenarios and the attack simulation | Bench host |
| **Model selection** | 4 models × 3 suites × 2 iterations | The real integration suites |
...
- **Scores versus thresholds.** The PII scrubber's recognizer confidences and its per-entity filter thresholds are one contract written in two places. A test parses both and fails if any recognizer can't clear its own threshold — the seam that once let four recognizers match and be silently discarded.
...
**Assertions are language-independent, and identities are disposable.** Scenarios assert on structural outcomes such as the login URL, never on English keywords, so a correct reply in Turkish or Arabic doesn't read as a failure. Each run gets a fresh identity, because a fixed test identity accumulates conversation history and abuse-guard strikes across runs until it starts failing for reasons that have nothing to do with the code — the security layer doing exactly its job against the test suite.
...
**What that produced in practice.** Every authentication path got enumerated, with an explicit verified/unverified marker and, where relevant, a recorded before-and-after account state. Seventeen paths were settled that way by handover, up from five. Several of the findings could not have come from any other method:
...
- **One upstream endpoint wasn't behaving as documented at the time** — it rejected valid, freshly issued codes on every attempt (reproduced with three separate request IDs, the last within 75 seconds of issue) while returning success on the request that should have sent them. That combination is what made it take a day to isolate rather than an hour.
...
I could have been writing tests for years; yet this is what finally made me articulate the difference between *tested* and *verified*.

### The upstream contract, as actually observed

Enumerating those paths turned up a set of upstream behaviours that no documentation stated and that each cost real debugging time. They're recorded here because they're the kind of thing a person taking over the integration would otherwise rediscover the hard way — and because most of them are general hazards of building against someone else's auth API, not quirks of this one.

| Observed behaviour | Why it bites | The rule it forced |
|---|---|---|
| An endpoint named to imply *"does this account exist"* actually answered *"is this number phone-verified"* | A perfectly normal `false` for a real customer reads as "no account" and locks them out — the exact bug that reached production | Treat that check as advisory only; it must never gate login, and it soft-fails to unknown on an upstream outage |
| The login link is a path segment; the signup link is a query parameter | Building one like the other yields a URL that looks right and doesn't work | The two links are constructed differently, and the difference isn't documented upstream |
| Five wrong passwords invalidate the whole login handle; five wrong OTP entries destroy the session | A blind retry loop burns the user's remaining attempts against a dead handle | On exhaustion the only recovery is a fresh login start, never a retry |
| Signing up onto a number that already has an account returns a conflict | Retrying the signup orphans the customer's existing eSIMs behind a duplicate account | On conflict, offer login instead — this is the duplicate-account trap |
| A token refresh re-reads verification state from the database before signing | A token minted seconds before verification would otherwise stay "unverified" for its whole life | Matters most at first login and at the 90-day session boundary |
| Logout clears and blacklists our side; the upstream doesn't revoke on its side | "Logged out" is only half true, and assuming otherwise leaves a working token in the wild | Treated as a known asymmetry, mitigated by our own revocation layer |
| One of the password-reset endpoints went missing from the upstream API mid-integration | A flow that worked yesterday returns a 404 today, with nothing in our code changed | Version the observed contract, and treat upstream endpoints as capable of disappearing without notice |
| A purchase endpoint's documented schema used snake_case (`customer_id`) while the deployed backend required camelCase (`customerId`) | The call validates against the docs and fails against the server, and the two never agree until you diff a real request | Trust the wire, not the spec: when documentation and behaviour diverge, the running server is the source of truth |
| Backend error bodies were reduced to a bare `400 Bad Request` during one debugging window | The single most useful signal for root-causing — the server's own description of what it rejected — disappears exactly when it's needed most | Preserve upstream error detail through your own layers; a generic 400 turns a five-minute fix into a five-hour one |

The through-line: **an auth API's method names encode its authors' mental model, not yours.** Verify what each one actually does to account state, once, by observation — then write the rule down where the next person will find it.
...
```mermaid
flowchart LR
    PR["Pull request"] --> L["ruff + mypy<br/>fail fast"]
    L --> B["Build once<br/>production target"]
    B --> REG["GHCR<br/>PR-scoped tag"]
    REG --> IT["Pull exact tag<br/>full stack · journey suite"]
    IT --> M["Merge to prod"]
    M --> CD["Build and tag<br/>latest · sha · timestamp"]
    CD --> SSH["SSH deploy script"]
    SSH --> HV{"Healthy, and running<br/>image ID equals<br/>pulled image ID?"}
    HV -- yes --> OK["Deploy confirmed"]
    HV -- no --> AL["Critical alert"]
```

**CI as a fail-fast quality gate.** Every pull request runs lint and static type checking first — if style or types are wrong, nothing else runs. Then the Docker image is built **once** — with the same model build arguments production uses — and pushed to the registry under a pull-request-scoped tag. The integration job pulls *that exact tag*, through a CI-specific compose override that forbids rebuilding, so the image under test is provably the image that was built. It spins up the full stack — relational database, cache, vector store — and runs the core journey suite against a live container rather than mocks. Aggressive disk cleanup runs on both sides to keep CI runners from exhausting storage.
...
**Scheduled jobs, and the window nothing serialises.**

| Schedule (UTC) | Job |
|---|---|
| Hourly | LLM-as-judge scoring of unreviewed production conversations |
| 02:00 | Refresh the security feature table, **then** retrain — in that order, because training on a stale table is worse than not training |
| 02:30 | Relational and vector-store backup |
| 03:00 | Analytics aggregation, watched by a staleness alert because its failure is invisible: every business panel keeps rendering, just with old numbers |

Three jobs land inside one hour, installed by two independent scripts that don't know about each other. Training time grows with the feature table, so the realistic failure is training still running when the backup starts — both contending for the same database on a box already near its memory budget. The rule written down for whoever runs it next: if backups start creeping up, move the training job, not the backup. A late model is harmless; a failed backup isn't.

**Housekeeping as code.** A weekly maintenance GitHub Actions workflow prunes old build artifacts, untagged images, and stale run logs — keeping a bounded set of recent production images. Small, but it's the difference between infrastructure that runs for a year and infrastructure that fills a disk in month three.
